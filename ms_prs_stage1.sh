...
module load plink/1.9-170906
  ...
  # filter ms discovery gwas by p value threshold
          pval_threshold=$1
          awk -v p="$pval_threshold" '{if($7<=p) {print}}' ../mschip_discovery_gwas/discovery_metav3.0.meta > prs1.txt
  
  # remove variants in extended MHC
          awk '{if (!(($1==6) && ($2>=25000000) && ($2<=35000000))) {print}}' prs1.txt> prs2.txt
  ...
  for i in 0.01 0.1 0.2 0.4 0.6 0.8 1 
          do
                  generate_prs $i
	  done
