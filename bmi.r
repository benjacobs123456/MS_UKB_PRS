...
selected_vars = selected_vars %>% filter(!is.na(cBMI))%>%
filter(!is.na(A_02)) %>%
...
bmi_drb15=c(AP,quantile(r,c(0.025,0.975)))
...
RERI=exp(coeffs[10]+coeffs[11]+coeffs[12])-exp(coeffs[10])-exp(coeffs[11])+1
AP=RERI/(exp(coeffs[10]+coeffs[11]+coeffs[12]))
bmi_a02=c(AP,quantile(r,c(0.025,0.975)))

bmi=data.frame(rbind(bmi_drb15,bmi_a02))
write_csv(bmi,"bmi_APs.csv")
df = data.frame("bmi_a02"=raw_bmi_aps_a02,"bmi_drb"=raw_bmi_aps_drb)
write_csv(df,"raw_bmi_hla_APs.csv")
