...
selected_vars = selected_vars %>% filter(!is.na(`Age when periods started (menarche).0.0`))%>%
filter(!is.na(A_02)) %>%
filter(!is.na(DRB1_15))
...
r = foreach(icount(trials), .combine=cbind) %dopar% {
  ...
  model = glm(data=df,
              ...
              `Townsend deprivation index at recruitment.0.0`+
                ...
                `Age when periods started (menarche).0.0`*DRB1_15,
...
RERI=exp(coeffs[9]+coeffs[10]+coeffs[11])-exp(coeffs[9])-exp(coeffs[10])+1
AP=RERI/(exp(coeffs[9]+coeffs[10]+coeffs[11]))
menarche_drb15=c(AP,quantile(r,c(0.025,0.975)))
...
r = foreach(icount(trials), .combine=cbind) %dopar% {
  ...
  model = glm(data=df,
              ...
              `Townsend deprivation index at recruitment.0.0`+
                ...
                `Genetic principal components.0.3`+
                `Genetic principal components.0.4`+
                `Age when periods started (menarche).0.0`*A_02,
              family=binomial(link="logit"))
  coeffs=as.numeric(coef(model))
  RERI=exp(coeffs[9]+coeffs[10]+coeffs[11])-exp(coeffs[9])-exp(coeffs[10])+1
  AP=RERI/(exp(coeffs[9]+coeffs[10]+coeffs[11]))}
raw_menarche_aps_a02 = r
model = glm(data=df,
            MS_status~`Age at recruitment.0.0`+
              `Place of birth in UK - north co-ordinate.0.0`+
              `Townsend deprivation index at recruitment.0.0`+
...
RERI=exp(coeffs[9]+coeffs[10]+coeffs[11])-exp(coeffs[9])-exp(coeffs[10])+1
AP=RERI/(exp(coeffs[9]+coeffs[10]+coeffs[11]))
menarche_a02=c(AP,quantile(r,c(0.025,0.975)))

menarche=data.frame(rbind(menarche_drb15,menarche_a02))
write_csv(menarche,"menarche_APs.csv")

df = data.frame("menarche_a02"=raw_menarche_aps_a02,"menarche_drb"=raw_menarche_aps_drb)
write_csv(df,"raw_menarche_hla_APs.csv")
