...
selected_vars = selected_vars %>% filter(!is.na(smoking))%>%
filter(!is.na(A_02)) %>%
...
smoking_drb15=c(AP,quantile(r,c(0.025,0.975)))
...
RERI=exp(coeffs[10]+coeffs[11]+coeffs[12])-exp(coeffs[10])-exp(coeffs[11])+1
AP=RERI/(exp(coeffs[10]+coeffs[11]+coeffs[12]))
smoking_a02=c(AP,quantile(r,c(0.025,0.975)))

smoking=data.frame(rbind(smoking_drb15,smoking_a02))
write_csv(smoking,"smoking_APs.csv")

df = data.frame("smok_a02"=raw_smoking_aps_a02,"smok_drb"=raw_smoking_aps_drb)
write_csv(df,"raw_smoking_hla_APs.csv")
